...
All 8 skills are monolithic workflows (93-529 lines). Shared sub-tasks — vault entity discovery, GitHub activity reading, mapping resolution, vault section writing — are duplicated across workflows with subtle drift. New skills must rewrite these patterns from scratch instead of composing from a library.
...
- **Capabilities** — small (50-150 lines), focused, independently invocable. Clear inputs and outputs. No user interaction. Always autonomous.
...
3. Loads that capability's SKILL.md via the Skill tool
...
This is lazy loading — the capability only enters context when the agent reaches that phase. Skipped phases (auto mode, tools unavailable) never load their capabilities.
...
**Capabilities do not reference other capabilities.** Keep composition flat — one level of indirection. If a workflow needs two capabilities in sequence, it calls both:
...
**`write-vault-section`** (~50-70 lines, 5+ consumers)
- Consumers: intervals-time-entry, freshbooks-time-entry, session-rollup, refine-daily-note, topic-pulse
- Append or replace a named section in a daily/topic note with idempotency
...
**`resolve-mappings`** (~80-100 lines, 3 consumers)
- Consumers: intervals-time-entry, freshbooks-time-entry, intervals-to-freshbooks
- Loads mapping files from `.cache/om/intervals-cache/`, applies CONTAINS-match logic, learns new mappings
- Handles multiple mapping types via `mapping_type` input
...
**Step 1: `discover-vault-entities`**
