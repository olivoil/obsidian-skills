...
1. **Discover screen recordings**:
   ```bash
   bash scripts/find-screenrecordings.sh "{date}"
   ```
...
3. **Match recordings** by time overlap:
   ```bash
   bash scripts/match-recordings.sh /tmp/screenrecs-{date}.json /tmp/rodecaster-{date}.json
   ```
...
6. **Determine audio source** for transcription:
   ...
   - `omarchy-only` → extract audio from screen recording:
     ```bash
     bash scripts/extract-audio.sh "{video.path}"
     ```
...
1. Run the download script:
   ```bash
   bash scripts/download-gdrive.sh "<url>"
   ```
...
```bash
bash scripts/find-screenshots.sh "{date}" "{start_time}" "{duration_secs}"
```
...
2. Transcribe just that clip (no prompt, fast since it's tiny):
   ```bash
   bash scripts/transcribe.sh /tmp/firstpass_$$.wav "{engine}"
   ```
...
2. Run the transcription script:
   ```bash
   WHISPER_PROMPT="{prompt from Phase 1.5}" bash scripts/transcribe.sh "<audio-file>" "<engine>"
   ```
...
```
Speaker labels from pyannote are generic (`SPEAKER_00`, `SPEAKER_01`). Replace with real names if participants are known from context, daily notes, or `.cache/om/intervals-cache/people-context.md`. Otherwise use `Speaker A`, `Speaker B`, etc.
...
```bash
bash scripts/compress.sh "<wav-file>"
```

**4b. Upload MP3 to Google Drive** (all modes with audio):
```bash
bash scripts/upload-gdrive.sh "<mp3-file>"
```
...
```bash
bash scripts/merge-av.sh "<video-file>" "<rodecaster-wav>"
```
...
```bash
bash scripts/upload-youtube.sh "<video-file>" "<meeting-title>" "<summary>" "<date>"
```
