...
3. Browser automation configured and able to run page-context scripts on the Intervals tab
...
2. **Infer repo→project mappings**: When a PR clearly matches a time entry's project, add to `.cache/om/intervals-cache/github-mappings.md`
...
- Add attendee names visible in the calendar block (especially client names from `.cache/om/intervals-cache/people-context.md`)
...
- correlate GitHub repos to project names from notes and mappings
- match calendar attendees to person notes
...
- resolve project→workType for each time entry
- correlate GitHub repos to projects
- match Outlook calendar events to projects
...
Use your browser automation tool with these scripts from `scripts/`:
...
1. Read the current cache: `.cache/om/intervals-cache/project-mappings.md`
...
```
1. Read .cache/om/intervals-cache/project-mappings.md
2. Append new section under "## Cached Work Types by Project"
3. Write updated content to .cache/om/intervals-cache/project-mappings.md
```
...
Use the same ENTRIES data that was sent to `fill-entries.js`. Right-align the Hours column and add a bold Total row.
...
```
$OBSIDIAN_VAULT_PATH/.cache/om/time-entries.db
```
...
Run via Bash:
```bash
sqlite3 "$OBSIDIAN_VAULT_PATH/.cache/om/time-entries.db" <<'SQL'
CREATE TABLE IF NOT EXISTS intervals_time_entries (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  date TEXT NOT NULL,
  project TEXT NOT NULL,
...
The cache at `.cache/om/intervals-cache/project-mappings.md` in your project:
...
The cache at `.cache/om/intervals-cache/github-mappings.md`:
- Gets created on first use from plugin template
- Gets updated when the assistant discovers repo→project associations from:
...
The cache at `.cache/om/intervals-cache/outlook-mappings.md`:
- Gets created on first use from plugin template
- Gets updated when the assistant discovers calendar→project associations from:
  - Meeting subjects matching project names
  - Attendees matching known people in `.cache/om/intervals-cache/people-context.md`
...
On first use in a new project, bootstrap shared state and then the assistant will:
1. Check if `.cache/om/intervals-cache/project-mappings.md` exists
2. Ensure `.cache/om/intervals-cache/*.md` exists
3. Use and update these local cache files going forward
